...
- **操作**（两种方案二选一）：
  ...
  **🔴 子代理上下文优化（推荐）**：在子代理 context 中告知标准搜索链路，避免子代理浪费时间做无效探测：
  - ✅ **正确做法**：context 中加 `"搜索链路：web_search(Parallel MCP) → web_extract(Parallel MCP) → 失败降级 browser_navigate(本地Chrome) + eval body.innerText。全部免费无配额限制。"`
  - ❌ **反例**：不告知 → 子代理可能用 browser_navigate→Google 慢慢搜

  **方案 B（传统 — 主代理顺序执行）**：
  - 🥇 **标准链路：`web_search(Parallel)` 搜背景 → `web_extract(Parallel)` 读正文** — 免费、无配额限制，对 thepaper.cn / guancha.cn / ithome.com 等多数新闻站有效
  - 🥈 提取失败（反爬/JS） → `browser_navigate(本地Chrome)` + `eval body.innerText`
  - 🥉 终极降级 → `execute_code` + `httpx` 直连新闻站（见参考文件 urllib-article-extraction.md）
...
- **限制**：
  ...
  - 给原文链接
  - 如果走降级方案（如 web_extract 失败切到 browser），报告标注 **⚠️ 降级运行**
...
### 降级回退链（2026-06-13 更新 — 新标准链路）

```
🥇 web_search(Parallel MCP) → 搜背景信息
🥇 web_extract(Parallel MCP) → 读正文（免费、Markdown 干净）
🥈 提取失败 → browser_navigate(本地Chrome) + eval body.innerText
🥉 终极降级 → execute_code + httpx 直连新闻源站
   (thepaper.cn / guancha.cn / ithome.com 等已验证可用，详见 references/urllib-article-extraction.md)
```

⚠️ **注意**：以上链路全部免费、无配额限制。不再有 Exa 402 问题。
...
4. **搜索策略**：最多 2 次 web_search(Parallel)，Tier 1（跨平台最高热度）优先。
5. **搜索链路（Parallel 免费 MCP）**：`web_search(Parallel)` → `web_extract(Parallel)` → 提取失败降级 `browser_navigate(本地Chrome)`。全部免费，无配额限制。不存在 Exa 402 问题。
...
| 11 | 未在子代理 context 中告知搜索链路 | ❌ 子代理可能用 browser_navigate→Google 慢慢搜 | 在 context 中明确写标准链路：web_search(Parallel)→web_extract→browser 降级 |
