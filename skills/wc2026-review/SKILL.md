...
关键区分：不是所有猜错了都要调模型。按以下三档处理：

| 性质 | 典型场景 | 处理方式 |
|:----|:---------|:--------|
| 🌪️ **方差/噪音** | xG大优但输球、门将超神、门柱横梁、单刀不进、💯预期进球2.5+输了 | **记录但不修正**。足球本质低得分运动，方差是比赛的一部分。记到 lessons-learned 但不改权重、不调模型 |
| 🔍 **方向性错误** | **预测A赢但B赢了**（方向反了） | **必须深挖根因**。按10维度回溯，判断哪一维判断错了。如果需要（比如发现了普遍规律），**用 skill_manage patch 写入 wc2026-predictor 的 SKILL.md** |
| ⚖️ **小偏差** | 比分错了但胜负方向对、概率偏差在±15%内 | 记录到 accuracy-tracking 统计即可 |
...
判断是临时因素还是普遍规律？
    ├─ 临时（红牌、核心伤退）→ 记录但不动技能
    └─ 普遍（某队打法克制、裁判尺度和某队风格不搭）→ patch 写入 wc2026-predictor
    ↓
如果需要更新技能：
  1. 读 wc2026-predictor 当前内容
  2. 找到合适的章节（如"特殊情景修正"或"10维度评分"）
  3. 用 skill_manage patch 写入
```
